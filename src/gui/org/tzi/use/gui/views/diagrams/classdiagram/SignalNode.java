 ...
 */
public class SignalNode extends ClassifierNode implements SortChangeListener {

	private final MAttribute[] attributes;
	
	private final String[] attributeValues;
        
    private static final String ENUMERATION = "\u00ABsignal\u00BB";
    
    SignalNode( MSignal enumeration, DiagramOptions opt ) {
    	super(enumeration, opt);
        attributes = enumeration.getAttributes().toArray(new MAttribute[0]);
    ...
    public MSignal getSignal() {
        return (MSignal)getClassifier();
    }
    ...
    protected void calculateAttributeRectSize(Graphics2D g, Rectangle2D.Double rect) {
    	calculateCompartmentRectSize(g, rect, attributeValues);
    }
	...
	protected void onDraw( Graphics2D g ) {
        ...
        if ( fOpt.isShowAttributes() ) {
            ...
            shapeG.setColor(textColor);
            drawCompartment(shapeG, y, attributeValues, new Color[attributeValues.length], roundedBounds);
        }
