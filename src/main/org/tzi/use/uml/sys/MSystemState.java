...
public final class MSystemState {
   ...
   */
  private DirectedGraph fWholePartRelationshipGraph;
  ...
  MSystemState(String name, MSystem system) {
    ...
    fWholePartRelationshipGraph = new DirectedGraphBase();
  }
  ...
  MSystemState(String name, MSystemState x) {
    ...
    fWholePartRelationshipGraph = x.fWholePartRelationshipGraph;
  }
  ...
  private void auxDeleteLink( MLink link ) {
    if( link instanceof MWholePartLink ){      
      fWholePartRelationshipGraph.removeEdge((MWholePartLink)link);
    }
  ...
  public MLink createLink( MAssociation assoc, List objects )
    throws MSystemException {
    MLink link;
    int aggregationKind = assoc.aggregationKind();
	...
	throw new MSystemException( "Cannot insert two linkobjects of the same type"
      ...
      link = createLinkObject( ( MAssociationClass ) assoc,
			       name, objects );
    }else if ( ( aggregationKind == MAggregationKind.AGGREGATION) || 
	       ( aggregationKind == MAggregationKind.COMPOSITION) ){
      //link is Aggregation or Composition
      MWholePartLink wholePartLink = new MWholePartLinkImpl( assoc, objects );
      link = (MLink) wholePartLink;
	...
	throw new MSystemException( "Link `" + assoc.name() 
				    ...
				    + ") already exist." );
      MObject source = (MObject) wholePartLink.source();
      MObject target = (MObject) wholePartLink.target();
      fWholePartRelationshipGraph.add(source);
      fWholePartRelationshipGraph.add(target);
      // the link is irreflexive    
      if (wholePartLink.isReflexive())
	throw new MSystemException("Object `" + source.name()
				   + "' cannot be a subsection of itself.");
      //check for SHARED OBJECT OF THE COMPOSION RELATIONSHIP
      if( (aggregationKind == MAggregationKind.COMPOSITION) && 
	  fWholePartRelationshipGraph.numIncomingEdges(target) != 0 )                
	throw new MSystemException("Object `" + target.name()
				   + "' is the part of other composition.");
      // check for cycles that might be introduced by adding the new
      // whole/part link    
      if (fWholePartRelationshipGraph.existsPath(target, source) )
	throw new MSystemException(
				   "Detected cycle in aggregation hierarchy. Object `"
				   + source.name()
				   + "' is already a subsection of `"
				   + target.name() + "'.");
      // silently ignore duplicates
      fWholePartRelationshipGraph.addEdge(wholePartLink);
      linkSet.add( link );
    } else {
