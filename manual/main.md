...
$\mathit{oclIsTypeOf}(T):\mathit{Boolean}$ evaluates to `true`
if the callee is an instance of type $T$.\
**Notation:** `self``.oclIsTypeOf(`$T$`)`

#### oclIsKindOf

$\mathit{oclIsKindOf}(T):\mathit{Boolean}$ evaluates to `true`
...
$+(y:\mathit{Real}):\mathit{Real}$ describes the sum of `self`
and $y$.\
**Notation:** `self``+`$y$
...
$<(y:\mathit{Real}):\mathit{Boolean}$ evaluates to `true`, if
...
$<(y:\mathit{Integer}):\mathit{Boolean}$ evaluates to `true`,
...
$\mathit{flatten}():\mathit{Set}(T')$. If `self` is a set of
...
$\mathit{flatten}():\mathit{Bag}(T')$. If `self` is a bag of
