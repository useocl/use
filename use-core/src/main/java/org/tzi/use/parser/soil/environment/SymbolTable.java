...
package org.tzi.use.parser.soil.environment;
...
public class SymbolTable {
	...
	public SymbolTable(VariableEnvironment variableEnvironment) {
		this();
		if (variableEnvironment.isObjectVisible()) {
			fVisibleState = variableEnvironment.getSystemState();
		}
		
		for (java.util.Map.Entry<String, org.tzi.use.uml.ocl.value.Value> entry : variableEnvironment.getCurrentMappings().entrySet()) {
			String varName = entry.getKey();			
			setType(varName, entry.getValue().type());
		}
	}
